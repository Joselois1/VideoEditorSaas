...
export default function EditorPage() {
  ...
  const triggerDownload = () => {
    ...
    const suffix = TOOL_LABELS[state.activeTool ?? "trim"];
  ...
  const renderPanel = () => {
    ...
    switch (state.activeTool) {
      case "trim":
        return <TrimPanel duration={video.duration} settings={state.trim} onChange={updateTrim} />;
      case "speed":
        return <SpeedPanel value={state.speed.value} onChange={updateSpeed} />;
      case "crop":
        return <CropPanel videoWidth={video.width} videoHeight={video.height} settings={state.crop} onChange={updateCrop} />;
      case "audio":
        return <AudioPanel settings={state.audio} onChange={updateAudio} />;
      case "compress":
        return <CompressPanel settings={state.compress} onChange={updateCompress} />;
      case "format":
        return <FormatPanel current={state.format.target} onChange={updateFormat} />;
      case "rotate":
        return <RotatePanel settings={state.rotate} onChange={updateRotate} />;
      case "color":
        return <ColorPanel settings={state.color} onChange={updateColor} />;
      case "reverse":
        return <ReversePanel settings={state.reverse} onChange={updateReverse} duration={video.duration} />;
      case "volume":
        return <VolumePanel settings={state.volume} onChange={updateVolume} />;
      case "fade":
        return <FadePanel settings={state.fade} onChange={updateFade} duration={video.duration} />;
      case "loop":
        return <LoopPanel settings={state.loop} onChange={updateLoop} duration={video.duration} />;
      case "extract-frame":
        return <ExtractFramePanel settings={state.extractFrame} onChange={updateExtractFrame} duration={video.duration} />;
      case "gif":
        return <GifPanel settings={state.gif} onChange={updateGif} duration={video.duration} />;
      case "fps":
        return <FpsPanel settings={state.fps} onChange={updateFps} />;
      case "join":
        return <JoinPanel originalName={video.name} additionalFiles={state.join.additionalFiles} onChange={updateJoin} />;
      case "text":
        return <TextPanel settings={state.text} onChange={updateText} duration={video.duration} />;
      case "noise":
        return <NoisePanel settings={state.noise} onChange={updateNoise} />;
      default:
        return null;
    }
  };
  ...
  const processLabel = () => {
    ...
    if (!isReady) return "Cargando motor...";
    if (state.activeTool === "extract-frame") return "Capturar frame";
    if (state.activeTool === "gif") return "Generar GIF";
    return "Procesar y descargar";
  };
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 flex flex-col gap-4">
                ...
                {state.outputUrl && (
                  <div className="bg-zinc-900 border border-green-500/30 rounded-xl p-4 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-green-400 font-medium text-sm">
                        {state.outputIsImage ? "Imagen lista" : "Video listo"}
                      </p>
                      <p className="text-zinc-500 text-xs mt-0.5">Procesado correctamente.</p>
                    </div>
                    <Button onClick={triggerDownload} size="md">Descargar</Button>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-zinc-900 border border-white/5 rounded-2xl p-4 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
                  <Toolbar activeTool={state.activeTool} onToolSelect={(t: ToolType) => setActiveTool(t)} />
